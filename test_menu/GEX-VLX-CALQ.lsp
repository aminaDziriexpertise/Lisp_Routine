...
(princ))
(princ)(defun c:Menu_GexCalq (/ fn acadobj thisdoc menus currmenugroup newmenu newmenuitem openmacro)
	(vl-load-com)
  ;Récupère le chemin vers le dossier support d'autocad
...
(setq flag nil)
  ...
  (if (not (findfile "GexCalq.mns"))

    (progn

      (setq fn (open "GexCalq.mns" "w"))
  ...
  ;(setq menufile (vla-item menus "VbaMenu"))
  (setq menufile (command "._MENULOAD" "GexCalq.mns"))
  ;Définition du menu
  (defun createmenu ()
    ;Titre du menu déroulant
    (setq newmenu (vla-add (vla-get-menus currmenugroup) "&VbaMenu"))
    ...
    ;Calques TopoRezo
    ;(setq openmacro (strcat (chr 3) (chr 3) "CalqInit" (chr 32) "256" (chr 32)))
    ;(setq newmenuitem (vla-addmenuitem newmenu (1+ (vla-get-count newmenu)) "TopoRezo" openmacro ))
    ;(vla-put-helpstring newmenuitem "Charger les calques de base pour TopoRezo")
    ;Calques NFP
    ;(setq openmacro (strcat (chr 3) (chr 3) "CalqInit" (chr 32) "512" (chr 32)))
    ;(setq newmenuitem (vla-addmenuitem newmenu (1+ (vla-get-count newmenu)) "NFP" openmacro ))
    ;(vla-put-helpstring newmenuitem "Charger les calques de base pour les normes parkings")
    ;Séparateur
    ;(vla-addseparator newmenu (1+ (vla-get-count newmenu)))
    ;Styles textes
    ;(setq openmacro (strcat (chr 3) (chr 3) "getLayHelp" (chr 32)))
    ;(setq newmenuitem (vla-addmenuitem newmenu (1+ (vla-get-count newmenu)) "Charte Calques (PDF)" openmacro ))
    ;(vla-put-helpstring newmenuitem "Affiche l'aide des calques")
    ;Séparateur
    ;(vla-addseparator newmenu (1+ (vla-get-count newmenu)))
    ;Styles textes
    ;(setq openmacro (strcat (chr 3) (chr 3) "StylesTXT" (chr 32)))
    ;(setq newmenuitem (vla-addmenuitem newmenu (1+ (vla-get-count newmenu)) "Styles textes" openmacro ))
    ;(vla-put-helpstring newmenuitem "Charger les styles de textes")
    ;Styles cotes
    ;(setq openmacro (strcat (chr 3) (chr 3) "StylesCOT" (chr 32)))
    ;(setq newmenuitem (vla-addmenuitem newmenu (1+ (vla-get-count newmenu)) "Styles cotes" openmacro ))
    ;(vla-put-helpstring newmenuitem "Charger les styles de cotes")
    ;Blocs codé
    ;(setq openmacro (strcat (chr 3) (chr 3) "BlocsCOD" (chr 32)))
    ;(setq newmenuitem (vla-addmenuitem newmenu (1+ (vla-get-count newmenu)) "Blocs codé" openmacro ))
    ;(vla-put-helpstring newmenuitem "Charger les blocs pour le levé codé")

    ;Insertion du menu à droite du menu "?"
    (vlax-for item (vla-get-menubar (vlax-get-acad-object))
      (setq lst (cons (strcase (vla-get-namenomnemonic item)) lst)) )
	(if (not (setq menu_pos (vl-position "?" (reverse lst))))
		(setq menu_pos (vla-get-count (vla-get-menuBar (vlax-get-acad-object)))))
    (vla-insertinmenubar newmenu (+ menu_pos 1))
    ;Re-compilation du menu
    (vla-save currmenugroup acmenufilecompiled)
    ;Enregistrement du menu
    (vla-save currmenugroup acmenufilesource)
  )
  ;Test du fichier mns
  (if (not (findfile menufile))
    (progn
      (setq fn (open menufile "w"))
      (close fn))
  )
  ;Verification si le menu est déjà chargé
  (setq menus (vla-get-menugroups (vlax-get-acad-object)))
  (vlax-for eachMG menus
    (if (= (vla-get-name eachMG) "GEXCALQ")
      (vla-unload (vla-item (vla-get-menugroups (vlax-get-acad-object)) "GexCalq")))
  )
  ;Chargement du menu
  (vla-load menus menufile)
  (setq currMenuGroup (vla-item menus "GexCalq"))
  ;Création du menu
